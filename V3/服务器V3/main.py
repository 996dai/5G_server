...
class App:
    ...
    def duo(self):
        while 1:
            ...
            datas = recData.decode("utf-8").split(',')
            shu = f'设备编号:{datas[0]},状态码:{datas[1]}'
            self.ent3_1.insert('end', shu + '\n')  # 显示接收到的数据
            self.ent1_1.insert('end', str(clientAddr))

            s = Aaa(str(recData.decode("utf-8")).split(',')[1],datas[0])
    ...
    def labels(self):
        Label(self.frame1, text="接收到的设备ip", font=('正楷', 18)).grid(column=0, row=0)
